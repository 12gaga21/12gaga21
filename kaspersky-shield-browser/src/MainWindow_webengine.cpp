#include "MainWindow_webengine.h"
...
void MainWindow::setupWebEngine()
{
    ...
    settings->setAttribute(QWebEngineSettings::DisallowUnknownUrlSchemes, true);
    ...
    settings->setAttribute(QWebEngineSettings::WebGL2Enabled, false);
    settings->setAttribute(QWebEngineSettings::AutoLoadIconsForPage, false);
    settings->setAttribute(QWebEngineSettings::TouchIconsEnabled, false);
    settings->setAttribute(QWebEngineSettings::FocusOnNavigationEnabled, true);
    settings->setAttribute(QWebEngineSettings::HyperlinkAuditingEnabled, false);
    settings->setAttribute(QWebEngineSettings::DefaultTextEncoding, "utf-8");
    settings->setAttribute(QWebEngineSettings::LocalStorageEnabled, true);
    settings->setAttribute(QWebEngineSettings::SessionStorageEnabled, true);
    settings->setAttribute(QWebEngineSettings::WebGLEnabled, false);
    settings->setAttribute(QWebEngineSettings::Accelerated2dCanvasEnabled, false);
    settings->setAttribute(QWebEngineSettings::PrintElementBackgrounds, false);
    settings->setAttribute(QWebEngineSettings::AllowRunningInsecureContent, false);
...
void MainWindow::setupMenuBar()
{
    ...
    fileMenu->addAction("&New Tab", this, &MainWindow::onNewTabRequested, QKeySequence::AddTab);
...
QWebEngineView* MainWindow::createNewTab(const QString &url)
{
    ...
    connect(page, &QWebEnginePage::downloadRequested, this, &MainWindow::onDownloadRequested);
    connect(page, &QWebEnginePage::certificateError, this, &MainWindow::onCertificateError);
...
void MainWindow::onDownloadRequested(QWebEngineDownloadItem *download)
{
    ...
    QMessageBox::StandardButton reply = QMessageBox::question(this, "Download",
        QString("Download %1?").arg(fileName),
        QMessageBox::Yes | QMessageBox::No);
    
    if (reply == QMessageBox::Yes) {
        download->accept();
        m_statusLabel->setText(QString("Downloading %1...").arg(fileName));
        m_statusTimer->start();
    } else {
        download->cancel();
    }
...
void MainWindow::onCertificateError(const QWebEngineCertificateError &error)
{
    ...
    QMessageBox::StandardButton reply = QMessageBox::warning(this, "Certificate Error",
        QString("Certificate error: %1\n\nDo you want to continue?").arg(error.errorDescription()),
        QMessageBox::Yes | QMessageBox::No);
